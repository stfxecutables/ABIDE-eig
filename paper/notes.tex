...
Functional magnetic resonance images (fMRI) is especially difficult. The 4D nature of the data means
that a single preprocessed fMRI image can be hundreds of megabytes: a single fMRI is a (3D) video
file\footnote{Except that while in a video file pixel values are in \([0, 255]\), in fMRI the voxel
values can be any floating point value.}. And while there are well-known and well-tested networks
that are freely-available pre-trained for processing 2D images (e.g. ResNet, ResNext, MobileNet),
models for processing even standard 2D video data are less well known, with the principles
underlying good design of 3D CNNs still just being investigated
\citep{xieRethinkingSpatiotemporalFeature2018,tranCloserLookSpatiotemporal2018,wangVideoModelingCorrelation2020}.
There are only a small number of models that process fMRI data in full [cites].
