...
selection (or engineering) procedure \(S: \mathbb{R}^n \rightarrow \mathbb{R}^m\), and a model (e.g. classifier)
...
\begin{enumerate}
  ...
  \item compute \(\omega^{\star} = \argmin_{\omega} \mathcal{L}(\hat{\theta}, \omega)\) [or instead \(\theta^{\star} = \argmin_{\theta} \mathcal{L}(\theta, \hat{\omega})\)]
  \item compute
  \(\theta^{\star} = \argmin_{\theta} \mathcal{L}(\theta, \omega^{\star})\) [or instead
  \(\omega^{\star} = \argmin_{\omega} \mathcal{L}(\theta^{\star}, \omega)\)]
...
As per above, there is nothing special about feature selection. Feature selection + tuning is just
another more complicated single fitting procedure, and so feature selection isn't \emph{exempt} from
the requirements of cross-validation or holdout. That is, in the notation of above, and putting back
in the data used for the loss calculations, then
...
is a biased and overfit estimate of performance (regardless if \(\mathcal{L}\) uses k-fold
internally) because such a procedure uses all the data multiple times, and simply chooses the
largest value. In particular, with the above procedure, there is no way to know if feature selection
or engineering has actually resulted in meaningful improvements (compared to no feature
selection/engineering). In fact, as most DL is viewed as performing implicit feature engineering,
choosing the "best" feature set and reporting results only for that feature set would be the
...
feature selection cannot be meaningfully compared to a study that makes proper use of a test set

Instead, one must report
...
Unfortunately, almost all studies using a large number of subjects and reporting overall accuracies
exceeding 71\% either involve such a questionable feature-selection procedure, or provide
insufficient details to rule out whether feature selection used all subjects (see
\ref{tab:existing-attempts}, "Biased" column).
